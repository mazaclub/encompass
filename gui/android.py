...
def make_layout(s, scrollable = False):
    content = """
      ...
      <LinearLayout 
        ...
        <TextView
          android:id="@+id/textEncompass"
          android:text="Encompass"
...
def pay_to(recipient, amount, fee, label):
    ...
    droid.dialogCreateSpinnerProgress("Encompass", "signing transaction...")
    droid.dialogShow()
        ...
        if event["name"] == "click":
            ...
            if id=="buttonPay":
                ...
                if not is_valid(recipient):
                    modal_dialog('Error','Invalid coin address')
                    continue
...
class ElectrumGui:
    ...
    def restore_wallet(self):
        ...
        droid.dialogCreateSpinnerProgress("Encompass", msg)
        droid.dialogShow()
